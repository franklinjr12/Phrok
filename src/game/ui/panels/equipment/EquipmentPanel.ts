...
import { compareEquipmentItems, equipmentSlotLabels, equipmentSlots, getEquipmentStats, getItemEquipmentSlot, removeEquipment } from "../../../systems/equipment";
...
export class EquipmentPanel implements UIPanel {
  ...
  render(): void {
    ...
    addPanelText(this.context, 96, 84, "Equipment", 24, "#f8fafc");
    ...
    addPanelText(this.context, 96, 120, `Attack ${derived.physicalAttack}   Defense ${derived.defense}   Gear ${this.getEquipmentBonusText(stats)}`, 15, "#bbf7d0");
    ...
    equipmentSlots.forEach((slot, index) => {
      ...
      const rowObject = addPanelRectangle(this.context, x, y, 254, 38, selected ? 0x293548 : 0x18222c, 0.94).setOrigin(0).setStrokeStyle(1, selected ? 0xfacc15 : 0x334155, 0.9).setInteractive({ useHandCursor: true });
      ...
      addPanelText(this.context, x + 12, y + 9, equipmentSlotLabels[slot], 14, "#94a3b8");
      addPanelText(this.context, x + 112, y + 9, item?.name ?? "Empty", 14, item ? "#f8fafc" : "#64748b");
    });
  ...
  private renderSelectedDetails(): void {
    ...
    addPanelRectangle(this.context, 96, 346, 386, 86, 0x17212b, 0.95).setOrigin(0).setStrokeStyle(1, 0x475569, 0.86);
    addPanelText(this.context, 112, 360, `${equipmentSlotLabels[this.selectedSlot]}: ${item?.name ?? "Empty"}`, 14, item ? "#f8fafc" : "#94a3b8");
    addPanelText(this.context, 112, 388, wrapText(`Effects: ${item ? this.getItemModifierText(item) : "None"}`, 48), 12, item ? "#bbf7d0" : "#64748b");
  }

  private renderComparisonFrame(): void {
    addPanelRectangle(this.context, 520, 346, 170, 138, 0x17212b, 0.95).setOrigin(0).setStrokeStyle(1, 0x475569, 0.86);
    addPanelText(this.context, 536, 362, "Comparison", 16, "#f8fafc");
    addPanelText(this.context, 536, 392, "Hover gear to compare.", 13, "#94a3b8");
  }
